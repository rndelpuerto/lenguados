...
export function defineSuite(): SuiteDefinition {
 ...
 return {
  ...
  setup(math2d) {
   ...
   const Rotation2 = math2d['Rotation2'] as any;
   const Matrix3 = math2d['Matrix3'] as any;
   const Transform2 = math2d['Transform2'] as any;
   const Complex = math2d['Complex'] as any;

   a = V2.fromValues(3.5, 7.2);
   b = V2.fromValues(1.1, 4.8);
   out = V2.fromValues(0, 0);
   unitA = V2.fromAngle(0.7);
   rot = Rotation2.fromAngle(0.5);
   m3 = Matrix3.fromRotation(0.7);
   t2 = Transform2.fromComponents(V2.fromValues(1, 2), 0.3, V2.fromValues(1, 1));
   c = Complex.fromPolar(1, 0.4);
   ...
   // Divide triality
   add('Vector2.divideScalar', () => V2.divideScalar(a, 2.5, out), 'default');
   add('Vector2.divideScalarSafe', () => V2.divideScalarSafe(a, 2.5, out), 'safe');
   add('Vector2.divideScalarUnchecked', () => V2.divideScalarUnchecked(a, 2.5, out), 'unchecked');
   ...
   add('Vector2.normalizeSafe (out)', () => V2.normalizeSafe(a, out), 'safe');
   add('Vector2.normalizeUnchecked (out)', () => V2.normalizeUnchecked(a, out), 'unchecked');

   // Direction triality
   add('Vector2.direction', () => V2.direction(a, b, out), 'default');
   add('Vector2.directionSafe', () => V2.directionSafe(a, b, out), 'safe');
   add('Vector2.directionUnchecked', () => V2.directionUnchecked(a, b, out), 'unchecked');
   ...
   // Projection triality
   add('Vector2.project', () => V2.project(a, b, out), 'default');
   add('Vector2.projectSafe', () => V2.projectSafe(a, b, out), 'safe');
   add('Vector2.projectUnchecked', () => V2.projectUnchecked(a, b, out), 'unchecked');
   add('Vector2.reject', () => V2.reject(a, b, out), 'default');
   add('Vector2.rejectSafe', () => V2.rejectSafe(a, b, out), 'safe');
   add('Vector2.rejectUnchecked', () => V2.rejectUnchecked(a, b, out), 'unchecked');

   // Reflect triality
   add('Vector2.reflect', () => V2.reflect(a, unitA, out), 'default');
   add('Vector2.reflectSafe', () => V2.reflectSafe(a, unitA, out), 'safe');
   add('Vector2.reflectUnchecked', () => V2.reflectUnchecked(a, unitA, out), 'unchecked');
   ...
   // Inverse triality
   add('Vector2.inverse', () => V2.inverse(a, out), 'default');
   add('Vector2.inverseSafe', () => V2.inverseSafe(a, out), 'safe');
   add('Vector2.inverseUnchecked', () => V2.inverseUnchecked(a, out), 'unchecked');

   // Set magnitude triality
   add('Vector2.setMagnitude', () => V2.setMagnitude(a, 5.0, out), 'default');
   add('Vector2.setMagnitudeSafe', () => V2.setMagnitudeSafe(a, 5.0, out), 'safe');
   add('Vector2.setMagnitudeUnchecked', () => V2.setMagnitudeUnchecked(a, 5.0, out), 'unchecked');
  },
