 ...
 *
 * Provides high-precision reference values for all 14 scalar deterministic
...
/** Complete list of all 14 scalar deterministic kernel functions */
...
export function generateInputs(fn: KernelFunction, count: number): number[][] {
 ...
 switch (fn) {
  ...
  case 'sinh':
  case 'cosh': {
   // Domain: all reals. Overflow near ±710.48 (same e^x growth as exp).
   for (let i = 0; i < count; i++) {
    const t = i / (count - 1);
    inputs.push([t * 1400 - 700]);
   }
   inputs.push([0], [-0], [1], [-1], [710], [711], [-745], [1e-10]);
   break;
  }

  case 'tanh': {
   // Domain: all reals. Saturates to ±1 beyond |x| ≈ 19.06.
   for (let i = 0; i < count; i++) {
    const t = i / (count - 1);
    inputs.push([t * 44 - 22]);
   }
   inputs.push([0], [-0], [1], [-1], [19.06], [-19.06], [20], [1e-10]);
   break;
  }
 }
