...
export const defineSuite = definePackageSuite({
 name: 'Matrix2',
 entity: 'Matrix2',
 extract: ['Matrix2', 'Vector2'],
 dimensions: {
  determinism: ['fdlibm', 'native'],
  tier: ['default', 'safe', 'unchecked'],
 },

 register(add, triality, { Matrix2: M2, Vector2: V2 }) {
  const a = M2.fromRotation(0.7);
  const b = M2.fromScale({ x: 2.0, y: 1.5 });
  const out = M2.fromValues(0, 0, 0, 0);
  ...
  // Inverse triality
  triality({
   default: ['Matrix2.inverse', () => M2.inverse(a, out)],
   safe: ['Matrix2.inverseSafe', () => M2.inverseSafe(a, out)],
   unchecked: ['Matrix2.inverseUnchecked', () => M2.inverseUnchecked(a, out)],
  });

  // Solve triality
  triality({
   default: ['Matrix2.solveLinearSystem', () => M2.solveLinearSystem(a, v, vOut)],
   safe: ['Matrix2.solveLinearSystemSafe', () => M2.solveLinearSystemSafe(a, v, vOut)],
   unchecked: [
    'Matrix2.solveLinearSystemUnchecked',
    () => M2.solveLinearSystemUnchecked(a, v, vOut),
   ],
  });
