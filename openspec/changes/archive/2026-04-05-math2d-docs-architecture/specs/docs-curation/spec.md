...
**Reason:** The `packages/math2d/docs/` directory no longer exists. Documentation is now served via the Docusaurus site, which has its own auto-generated sidebar navigation that supersedes any manual index file.

**Migration:** Contributors find documentation via the Docusaurus site sidebar or the README.md Documentation section links.

### Requirement: Obsolete research files are archived

**Reason:** Rather than maintaining an archive directory within the package, all research files are removed from the working tree entirely. Git history serves as the permanent archive — `git log --all -- packages/math2d/docs/research/archive/` recovers any file.

**Migration:** Historical research files are recoverable via `git log` and `git show`. No separate archive directory is maintained.
