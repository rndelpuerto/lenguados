...
**Reason:** The package-level `packages/math2d/CONTRIBUTING.md` is being removed. Its unique content (~20 lines: suffix naming convention, `Readonly*Like` POJO interop example, fluent chaining rule) is absorbed into the root `CONTRIBUTING.md` math2d API Conventions section. Detailed guides (TSDoc standard, testing strategy, design philosophy) move to Docusaurus.

**Migration:** Contributors use root `CONTRIBUTING.md` for all contribution guidance. The "Detailed Guides" section links to Docusaurus pages for depth content that was previously in the package-level CONTRIBUTING.md or in `packages/math2d/docs/`.
