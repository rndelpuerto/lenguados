...
- `MIN_SAFE_DIVISOR` (= EPSILON, 1e-10): Safe division guard (`divideSafe`, `reciprocalSafe`, and functions that delegate to them)
...
#### Scenario: divideSafe documents MIN_SAFE_DIVISOR usage

- **WHEN** a developer reads the JSDoc for `divideSafe(numerator, denominator, epsilon?)`
- **THEN** the `@remarks` tag SHALL state: "Uses MIN_SAFE_DIVISOR (= EPSILON, 1e-10) as the minimum divisor threshold"
