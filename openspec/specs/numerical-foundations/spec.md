...
### Requirement: inverseLerp/inverseLerpSafe division guard uses exact zero check

The `inverseLerp` and `inverseLerpSafe` functions SHALL use `=== 0` for their zero-denominator guards. For scalar arithmetic operations in the auxiliary layer, exact zero is the only mathematically undefined input — any non-zero denominator produces a well-defined, representable quotient. The safe variant guards the same condition as the strict variant; only the response differs (fallback vs throw). This is consistent with `floorDivide`/`floorDivideSafe` and `remap`/`remapSafe` in the same layer.

**Source:** `packages/math2d/src/auxiliary/scalar/interpolation.ts:89-111`

**Reference:** IEEE 754-2019 Section 5.4.1 on division semantics; consistency with scalar arithmetic layer convention.
...
#### Scenario: Safe inverseLerpSafe returns fallback on exact zero

- **GIVEN** `a = 5`, `b = 5`, `value = 5`
- **WHEN** `inverseLerpSafe(a, b, value)` is called
- **THEN** the result SHALL be `0` (safe fallback), because `denominator === 0`

#### Scenario: Safe inverseLerpSafe computes valid small-denominator results

- **GIVEN** `a = 0`, `b = 1e-12`, `value = 3e-13`
- **WHEN** `inverseLerpSafe(a, b, value)` is called
- **THEN** the result SHALL be `0.3`, because the denominator `1e-12` is non-zero and the quotient is valid
...
### Requirement: Interval.divide uses exact zero check for both strict and safe

`Interval.divide` (strict variant) and `Interval.divideSafe` SHALL both use `scalar === 0` as their zero check. For scalar division of interval bounds, exact zero is the only undefined input — any non-zero scalar produces valid, representable results. The safe variant guards the same condition as the strict variant; only the response differs (fallback vs throw). This is consistent with `inverseLerp`/`inverseLerpSafe` and other scalar arithmetic in the auxiliary layer.

**Source:** `packages/math2d/src/core/interval.ts:423-451`

**Reference:** IEEE 754-2019 Section 7.3 on division by zero; consistency with scalar arithmetic convention.
...
#### Scenario: Safe Interval.divideSafe returns fallback on exact zero

- **GIVEN** `interval = Interval(1, 5)` and `scalar = 0`
- **WHEN** `Interval.divideSafe(interval, scalar)` is called
- **THEN** the result SHALL be `Interval(0, 0)` (safe fallback)
