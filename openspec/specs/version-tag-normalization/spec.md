...
#### Scenario: Vector2.sumComponents deprecation removed

- **WHEN** `Vector2.sumComponents` was previously marked `@deprecated`
- **THEN** the deprecation has been removed — `sumComponents` is retained as a non-deprecated core primitive (decision: math2d is a low-level math library; component sum is a fundamental reduction)
