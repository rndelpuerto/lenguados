# @lenguados/math2d Documentation Standard

This document is the authoritative reference for TSDoc documentation conventions in the `@lenguados/math2d` package. Every source file in `packages/math2d/src/` SHALL conform to this standard.
...
- [OpenSpec spec](../../openspec/specs/documentation-standard/spec.md) — Formal requirements with testable scenarios
...
⁴ Required for constants with non-obvious values (e.g., `MIN_SAFE_DIVISOR`).
⁵ Only for complex algorithms (e.g., `complexDivideSmith`).
⁶ Required for conversion constants (e.g., `DEG_TO_RAD`); optional for trivial constants (e.g., `PI`).
...
- `@file`: path relative from `src/` (e.g., `core/vector2.ts`)
- `@module`: full NPM path (e.g., `@lenguados/math2d/core`)
 ...
 * Cross-references to external standards or engines.
 *
 * @example
 * ```typescript
...
- `@remarks` if the value needs justification (e.g., `MIN_SAFE_DIVISOR`, `EPSILON`)
- `@example` if the constant is used in arithmetic (e.g., conversion factors); omit for trivial constants (e.g., `PI`)
...
- Summary uses imperative voice: "Clamps", "Returns", "Tests" (NOT "This function clamps")
...
- Summary includes fallback behavior (e.g., "Returns (0,0) if v has zero length")
...
**Gold standard:** `normalizeSafe` (vector2.ts), `remapSafe` (arithmetic.ts)
...
- `@remarks` only for complex algorithms (e.g., Smith's division, fdlibm kernels)
...
| `Set Operations`        | Interval            | `union`, `intersection`, `contains`, `overlaps`                                 |
...
| `@category`      | Members                                                                      |
| ---------------- | ---------------------------------------------------------------------------- |
| `Tolerance`      | `EPSILON`, `EPSILON_SQUARED`, `ITERATIVE_TOLERANCE`                          |
| `Angular`        | `PI`, `TAU`, `HALF_PI`, `QUARTER_PI`                                         |
| `Conversion`     | `DEG_TO_RAD`, `RAD_TO_DEG`, `RAD_TO_TURN`, `TURN_TO_RAD`                     |
| `Mathematical`   | `SQRT_2`, `SQRT_HALF`, `LN_2`, `E`, `GOLDEN_RATIO`, `GOLDEN_RATIO_CONJUGATE` |
| `Numeric Limits` | `MAX_SAFE_INTEGER_F64`, `SMALLEST_NORMAL`                                    |
...
The following are **deprecated** and SHALL be replaced during audit:
...
| -------------------------------------- | ----------------------------------------------------------- | ------ |
| Fields before constructor              | Instance Props + Static Constants first                     | ✓      |
| Constructor before methods             | Constructor after fields, before all methods                | ✓      |
| Static methods before instance methods | Static Factories→...→Comparison before Instance Getters→... | ✓      |
| Accessors grouped before methods       | Instance Accessors before Instance Mutators                 | ✓      |
| Public before protected before private | Natural (all public except Private Helpers)                 | ✓      |

**Section omission:** Sections with no members SHALL be omitted entirely (no empty dividers).

**Type-specific sections** appear after the last base section in their group (e.g., "Static Matrix Operations" after "Static Comparison") and before the next group starts.
...
- Files with multiple thematic groups (e.g., `constants.ts` has Tolerance, Angular, Conversion, Mathematical)
- Files with re-export sections (e.g., `safety.ts` has "Re-exports from deterministic-kernels")
- Utils files organized by type (e.g., `parse.ts` has sections per core type)

**When to omit section dividers:**

- Files with a single theme (e.g., `comparison.ts` — all functions are comparisons)
...
### Symbol and Formula Conventions

- **Unicode permitted:** θ, π, φ, ≈, ±, ∞, √ in prose
- **LaTeX prohibited:** Do not use `\frac{}{}`, `\sqrt{}`, etc.
