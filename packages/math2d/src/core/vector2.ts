...
import { isNearZero, nearEquals as scalarNearEquals } from '../auxiliary/scalar/comparison';
...
export class Vector2 implements Vector2Like {
 ...
 public static mod(a: ReadonlyVector2Like, b: ReadonlyVector2Like, out?: Vector2): Vector2 {
  return this.ensureOut(out).set(scalarMod(a.x, b.x), scalarMod(a.y, b.y));
 }
 ...
 public static modScalar(v: ReadonlyVector2Like, s: number, out?: Vector2): Vector2 {
  return this.ensureOut(out).set(scalarMod(v.x, s), scalarMod(v.y, s));
 }
 ...
 public static applyRotation(
  ...
  return Vector2.rotateCS(v, rotation.c, rotation.s, out);
 }
  ...
  * @param a - First vector.
  * @param b - Second vector.
  * @returns True if components are identical.
  *
  * @category Comparison
  * @since 0.8.0
  */
 public static equals(a: ReadonlyVector2Like, b: ReadonlyVector2Like): boolean {
  return a.x === b.x && a.y === b.y;
 }
  ...
  * @param b - Second vector.
  * @param epsilon - Tolerance. @defaultValue `EPSILON`
  * @returns True if both component differences are within epsilon.
  *
  * @category Comparison
  * @since 0.8.0
  */
 public static nearEquals(
  ...
  return scalarNearEquals(a.x, b.x, epsilon) && scalarNearEquals(a.y, b.y, epsilon);
 }
 ...
 public static isParallel(
  ...
  return scalarAbs(Vector2.cross(a, b)) <= epsilon;
 }
 ...
 public static isPerpendicular(
  ...
  return scalarAbs(Vector2.dot(a, b)) <= epsilon;
 }
 ...
 public mod(v: ReadonlyVector2Like): this {
  this.x = scalarMod(this.x, v.x);
  this.y = scalarMod(this.y, v.y);
  return this;
 }
 ...
 public modScalar(s: number): this {
  this.x = scalarMod(this.x, s);
  this.y = scalarMod(this.y, s);
  return this;
 }
  ...
  */
 public nearEquals(v: ReadonlyVector2Like, tolerance = EPSILON): boolean {
  return scalarNearEquals(this.x, v.x, tolerance) && scalarNearEquals(this.y, v.y, tolerance);
 }
 ...
 public applyRotation(rotation: ReadonlyRotation2Like): this {
  const rx = rotation.c * this.x - rotation.s * this.y;
  const ry = rotation.s * this.x + rotation.c * this.y;
  return this.set(rx, ry);
 }
