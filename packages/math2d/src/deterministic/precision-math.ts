...
import type { CompensatedResult, TwoSumResult, TwoProductResult } from './types';
...
export class PrecisionMath {
 ...
 static kahanSum(values: readonly number[]): number {
  ...
  for (const value of values) {
   const sanitized = this.sanitize(value, 'PrecisionMath.kahanSum:value');
 ...
 static neumaierSum(values: readonly number[]): number {
  ...
  for (const value of values) {
   const sanitized = this.sanitize(value, 'PrecisionMath.neumaierSum:value');
 ...
 static twoSum(a: number, b: number): TwoSumResult {
  const sanitizedA = this.sanitize(a, 'PrecisionMath.twoSum:a');
  const sanitizedB = this.sanitize(b, 'PrecisionMath.twoSum:b');
 ...
 static fastTwoSum(a: number, b: number): TwoSumResult {
  const sanitizedA = this.sanitize(a, 'PrecisionMath.fastTwoSum:a');
  const sanitizedB = this.sanitize(b, 'PrecisionMath.fastTwoSum:b');
  const sum = a + b;
 ...
 static twoProduct(a: number, b: number): TwoProductResult {
  const sanitizedA = this.sanitize(a, 'PrecisionMath.twoProduct:a');
  const sanitizedB = this.sanitize(b, 'PrecisionMath.twoProduct:b');
 ...
 static compensatedProduct(values: readonly number[]): CompensatedResult {
  ...
  let product = this.sanitize(values[0]!, 'PrecisionMath.compensatedProduct:value');
  let errorSum = 0;

  for (let index = 1; index < values.length; index++) {
   const sanitized = this.sanitize(values[index]!, 'PrecisionMath.compensatedProduct:value');
 ...
 static compensatedDot(a: readonly number[], b: readonly number[]): CompensatedResult {
  ...
  for (let index = 0; index < a.length; index++) {
   const sanitizedA = this.sanitize(a[index]!, 'PrecisionMath.compensatedDot:a');
   const sanitizedB = this.sanitize(b[index]!, 'PrecisionMath.compensatedDot:b');
 ...
 static extendedSum(values: readonly CompensatedResult[]): CompensatedResult {
  // Sum primary values
  const primarySum = this.kahanSum(
   values.map((v) => this.sanitize(v.value, 'PrecisionMath.extendedSum:value')),
  );

  // Sum error terms
  const errorSum = this.kahanSum(
   values.map((v) => this.sanitize(v.error, 'PrecisionMath.extendedSum:error')),
  );
