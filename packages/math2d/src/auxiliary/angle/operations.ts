...
import { abs } from '../scalar/arithmetic';
import { EPSILON, HALF_PI } from '../scalar/constants';
...
/**
 * Convergence tolerance for iterative angle algorithms.
 * @internal
 */
const ITERATIVE_TOLERANCE = 1e-6;
 ...
 * @remarks Used to avoid computing both separately when both are needed.
 */
...
export function sinCosInto(angle: number, out: SinCos): SinCos {
 out.sin = DeterministicMath.sin(angle);
 out.cos = DeterministicMath.cos(angle);
 return out;
}
...
export function sinCosNormalized(angle: number): SinCos {
 ...
 return {
  sin: DeterministicMath.sin(normalized),
  cos: DeterministicMath.cos(normalized),
 };
}
...
export function angleDistance(a: number, b: number): number {
 return abs(angleDifference(a, b));
}
...
export function angleAverage(angles: number[]): number {
 ...
 for (const angle of angles) {
  sumX += DeterministicMath.cos(angle);
  sumY += DeterministicMath.sin(angle);
 }

 // Return angle of average vector
 return DeterministicMath.atan2(sumY, sumX);
}
...
export function angleWeightedAverage(angles: number[], weights: number[]): number {
 ...
 for (let index = 0; index < angles.length; index++) {
  ...
  sumX += DeterministicMath.cos(angle) * weight;
  sumY += DeterministicMath.sin(angle) * weight;
 }
...
export function principalAngle(angles: number[]): number {
 ...
 for (let iter = 0; iter < maxIterations; iter++) {
  ...
  for (const angle of angles) {
   ...
   const weight = 1 / (abs(diff) + INVERSE_WEIGHT_SMOOTHING);
   sumX += DeterministicMath.cos(angle) * weight;
   sumY += DeterministicMath.sin(angle) * weight;
  }

  const newPrincipal = DeterministicMath.atan2(sumY, sumX);
  if (abs(angleDifference(principal, newPrincipal)) < ITERATIVE_TOLERANCE) {
   break;
  }
...
export function angleFromVectors(x1: number, y1: number, x2: number, y2: number): number {
 const angle1 = DeterministicMath.atan2(y1, x1);
 const angle2 = DeterministicMath.atan2(y2, x2);
...
export function isQuadrantAngle(radians: number, epsilon: number = EPSILON): boolean {
 ...
 const nearest = DeterministicMath.round(quarterTurns);
 return abs(quarterTurns - nearest) * HALF_PI <= epsilon;
}
